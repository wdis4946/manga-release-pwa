...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'manga_series_items_series_category_display_order_key'
      and conrelid = 'public.manga_series_items'::regclass
  ) then
    alter table public.manga_series_items
      add constraint manga_series_items_series_category_display_order_key
      unique (series_id, category_number, display_order)
      deferrable initially immediate;
  end if;
end;
$$;

drop index if exists public.manga_series_items_series_category_idx;

create index if not exists manga_series_items_series_category_idx
  on public.manga_series_items(series_id, category_number, display_order, isbn);

create or replace function public.assign_manga_series_item_display_order()
returns trigger
language plpgsql
security definer
...
begin
  if new.display_order is null
    or exists (
      select 1
      from public.manga_series_items as existing
...
drop trigger if exists manga_series_item_display_order_trigger
  on public.manga_series_items;
create trigger manga_series_item_display_order_trigger
  before insert on public.manga_series_items
  for each row
  execute function public.assign_manga_series_item_display_order();

create or replace function public.reorder_manga_series_items_category(
  p_series_id uuid,
...
begin
  ...
  from (
    ...
    from public.manga_series_items as moved
    cross join (
  ...
  where manga_series_items.series_id = p_series_id
    and manga_series_items.isbn = destination_order.isbn;
  ...
  loop
    perform public.reorder_manga_series_items_category(
      p_series_id,
      v_category_number
    );
  end loop;
...
create or replace function public.update_manga_series_item_display_orders(
  p_series_id uuid,
  p_item_orders jsonb
)
returns integer
...
begin
  ...
  if exists (
    ...
    where not exists (
      select 1
      from public.manga_series_items as item
      where item.series_id = p_series_id
  ...
  if exists (
    ...
    where not exists (
      select 1
      from public.manga_series_categories as category
...
revoke all on function public.assign_manga_series_item_display_order()
  from public, anon, authenticated;
revoke all on function public.reorder_manga_series_items_category(uuid, integer)
  from public, anon, authenticated;
revoke all on function public.update_manga_series_item_display_orders(uuid, jsonb)
  from public, anon, authenticated;

grant execute on function public.assign_manga_series_item_display_order()
  to service_role;
grant execute on function public.reorder_manga_series_items_category(uuid, integer)
  to service_role;
grant execute on function public.update_manga_series_item_display_orders(uuid, jsonb)
  to service_role;
