...
begin
  ...
  select
    ...
    public.normalize_manga_title(series.search_title, false)
      as series_normalized_title,
    similarity(
      public.normalize_manga_title(series.search_title, false),
      item.item_normalized_title
    ) as similarity_score,
    case
      when public.normalize_manga_title(series.search_title, false)
        = item.item_normalized_title
        then 'auto_title_exact'
      else 'auto_title_similarity'
    end as match_method
  from auto_link_items as item
  join public.manga_series as series
    on item.item_normalized_title is not null
   and (
      public.normalize_manga_title(series.search_title, false)
        = item.item_normalized_title
      or (
        char_length(item.item_normalized_title) >= p_min_similarity_length
        and similarity(
          public.normalize_manga_title(series.search_title, false),
          item.item_normalized_title
  ...
  select
    ...
    max(candidate.similarity_score) as best_similarity_score
  from auto_link_items as item
